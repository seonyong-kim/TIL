...
- The database는 각 files들의 모음으로 저장된다.
  ...
  - 각 file은 fixed-length storage units인 blocks으로 분할된다. (block은 저장 할당과 데이터 전송의 단위)
...
- Simple approach:
  - 각 record i는 byte n * (i– 1)위치 부터 저장하기 시작한다.
  - Record 접근은 간단하지만, records가 block boundaries넘지 않도록 허용하지 않는다.
- record i 삭제:
...
- Variable-length records arise in database systems in several ways:
  - 하나의 file에 multiple record types
  - one or more fields(예: 문자열 varchar)의 길이가 variable lengths
  - repeating fields(배열이나 멀티셋)를 허용<br>
...
## Variable-Length Records: Slotted Page Structure 여기서 부터 다시 공
